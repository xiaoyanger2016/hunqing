...
class RegionController extends AdminbaseController {
    ...
    protected $region_mod;

    function _initialize() {
        parent::_initialize();
        $this->region_mod = D("Common/Region");
       // $this->menu_model = M("Region");
    }
    ...
    function index()
    {
    	...
    	if(!empty($region_name)){
    		$where['region_name'] = array('like',$region_name);
    	}
    	$this->assign('region_name', $region_name);
    	//分页
    	$count=$this->region_mod->where($where)->count();
    	$page = $this->page($count, 20);
    	//查询list
    	$list = $this->region_mod
		    	->where($where)
		    	->order(array("sort_order" => "ASC","region_id" => "ASC"))
    	...
    	{
    		$list[$key]['switchs'] = 0;
    		if ($this->region_mod->get_list($val['region_id']))
    		{
    			$list[$key]['switchs'] = 1;
    		}
    		$list[$key]['str_manage'] = '<a href="' . U("Region/edit", array("id" => $val['region_id'], "menuid" => I("get.menuid"))) . '">'.L('EDIT')
    		.'</a> | <a class="js-ajax-delete" href="' . U("Region/delete", array("id" => $val['store_id'], "menuid" => I("get.menuid")) ). '">'.L('DELETE').'</a> ';
